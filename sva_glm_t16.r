...
combat_expr_data$gender = patient_info$gender
x = as.matrix(combat_expr_data)
...
for(i in 0:9)
{
  ...
  png_f = paste("D:/R_workspace/survival/PPT/cv_full_features/",i,"full_features.png",sep = "")
  ...
  for(k in 1:length(cv_Folds))
  {
    ...
    text(60,0.85,sprintf("p-value of log-rank test: %.10f", log_rank_predict_a_pvalue),col='brown2')
