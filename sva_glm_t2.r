...
## gender batch 
#batch_gender = t1$gender

#combat_data_protocol_gender = ComBat(dat=combat_data_protocol, batch=batch_gender, mod=modcombat, par.prior=TRUE, prior.plots=TRUE)

## median scale
combat_data_protocol_gender = combat_data_protocol
aft_data = combat_data_protocol_gender
...
scale_median_data =  apply(combat_data_protocol_gender,2,scale_median_3)
...
set.seed(123589)
cvfit_3 = cv.glmnet(x, y, family = "gaussian",maxit=15000,alpha=0.4,nlambda = 150)
plot(cvfit_3)
...
fit_04 = glmnet(x, y, family="gaussian", alpha=0.4,lambda = 1.5)
