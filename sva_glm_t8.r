...
t1$china2008.M[is.na(t1$china2008.M)] = 0
batch[batch=='anchordx' & t1$gender==1 & t1$china2008.M==0] = 1
batch[batch=='anchordx' & t1$gender==1 & t1$china2008.M==1] = 2
batch[batch=='anchordx' & t1$gender==2 & t1$china2008.M==0] = 3
batch[batch=='anchordx' & t1$gender==2 & t1$china2008.M==1] = 4

batch[batch=='novogene' & t1$gender==1 & t1$china2008.M==0] = 5
batch[batch=='novogene' & t1$gender==1 & t1$china2008.M==1] = 6
batch[batch=='novogene' & t1$gender==2 & t1$china2008.M==0] = 7
batch[batch=='novogene' & t1$gender==2 & t1$china2008.M==1] = 8
...
# plot(cvfit_2)
# 
# cvfit_3 = cv.glmnet(x, y, family = "gaussian",maxit=15000,alpha=0.42,nlambda = 150)
# plot(cvfit_3)
#chosen_index = rownames(coefficients)[which(coefficients != 0)]


fit_04 = glmnet(x, y, family="gaussian", alpha=0.42,lambda = 1.0020)
...
#write.csv(chosen_features,'D:/R_workspace/survival/PPT/chosen_features_2.csv')
...
pdf('D:/R_workspace/survival/PPT/test2.pdf',width=15,height = 12)
for(k in 1:10){
