...
for(k in 100:1000)
{
  cvfit_test = cv.glmnet(x, y, family = "cox",alpha=1/k,nlambda = 150)
  fit_test = glmnet(x, y, family="cox", alpha=1/k,lambda = cvfit_test$lambda.min)
  if(fit_test$dev.ratio>0.4){break}
}
