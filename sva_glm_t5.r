...
pdf('D:/R_workspace/survival/PPT/das.pdf')
for(k in 1:10){
  ...
  data_a$predict_a[data_a$predict_a>39] = 0
  
  data_b$predict_data = data_b$predict_b
  data_b$predict_b[data_b$predict_b<=39] = 1
  data_b$predict_b[data_b$predict_b>39] = 0
  ...
  par(mfrow=c(2,1))
  survplot(km_a,xlab = 'months', n.risk   = TRUE,cex.n.risk = 1,type="kaplan-meier",lwd=2,
  ...
  survplot(km_b,xlab = 'months', n.risk   = TRUE,cex.n.risk = 1,type="kaplan-meier",lwd=2,
           label.curves = list(method = "arrow", cex = 1.2),col=c(1:4),abbrev.label=TRUE)
  text(90,0.8,sprintf("p-value of log-rank test: %.10f", log_rank_predict_b_pvalue),col='brown2')
  title(title_b)
  print(table(data_a$predict_a,data_a$y_a_status))
  print(table(data_b$predict_b,data_b$y_b_status))
}
...
p + geom_abline(intercept = 0, slope = 1,col = 'red',size=1.5)+
  geom_smooth(method=lm)+geom_hline(aes(yintercept=39),col='blue', linetype="dashed",size=2)



SurvObj <- with(t1, Surv(survival.month, cencor.status == 1))

km_test = npsurv(SurvObj ~ testData_y, data = t1)



log_rank_gender=survdiff(SurvObj~gender, data=t1)

log_rank_cencor.status = survdiff(SurvObj~cencor.status, data=t1)

log_rank_cencor.status_pvalue = pchisq(log_rank_cencor.status$chisq, length(log_rank_cencor.status$n)-1, lower.tail = FALSE)

ba2 = npsurv(SurvObj ~ cencor.status, data = t1)

survplot(ba2,xlab = 'months', n.risk   = TRUE,cex.n.risk = 1,type="kaplan-meier",
         label.curves = list(method = "arrow", cex = 1.2),col=c(1:4),abbrev.label=TRUE)
