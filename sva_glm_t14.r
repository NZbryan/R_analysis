...
# png("D:/R_workspace/survival/PPT/cv_Folds3_3.png",width = 800,height = 1000)
...
for(k in 1:length(cv_Folds))
{
  ...
  cvfit_cox = cv.glmnet(x, y, family = "cox",alpha=0.01,nlambda = 150)
 # plot(cvfit_cox)
  
  ## train the model
  fit_cox = glmnet(train_x, train_y, family="cox",alpha=0.01,lambda = cvfit_cox$lambda.min)
...
ggplot(data = data_a, aes(x = log(y_predict)))+geom_histogram(binwidth=0.2,color='blue')+
  geom_vline(aes(xintercept=log(median(data_a$y_predict))), col='red', linetype="dashed",size=2)
