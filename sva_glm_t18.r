...
for(i in 0:9)
{
  ...
  for(k in 1:length(cv_Folds))
  {
    ...
    ## train the model
    fit_cox = glmnet(train_x, train_y, family="cox",alpha=0.15,lambda = cvfit_cox$lambda.min)
...
chosen_features = read.csv('D:/R_workspace/survival/PPT/PPT/chosed_p_hr_beta.csv',row.names = 1)
